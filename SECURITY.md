...
Supervised Team Runs require a persisted daemon password because their restricted provider processes can share loopback reachability with trusted clients. `PASEO_PASSWORD` does not qualify: on same-user hosts, a provider process may read the daemon ancestor's startup environment even though the variable is not inherited directly.
