...
- See the [configuration guide](../public-docs/configuration.md#apply-changes) for relay opt-in and reload behavior. `DaemonConfigStore` persists the desired state, while the relay runtime starts or stops the outbound transport live; pairing reads that current state instead of a startup snapshot.
