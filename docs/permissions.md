...
Before enabling device authentication, integrate admission across direct HTTP, WebSocket, and relay connections. Revalidate open sessions when a credential is revoked or its principal's grants change; checking only the handshake leaves existing connections authorized. Bootstrap approval must come from a trusted local path, never from an anonymous socket requesting owner authority.

The staged activation primitive records the opt-in separately in `device-authentication.json`, after an enrolled credential has `access.manage`. Enrollment alone does not opt in. Transport adapters must consult that record before choosing their authentication policy, and fail closed if it cannot be read. Losing `device-access.json` after activation is a storage failure, not permission to recreate an empty registry or admit anonymous clients. Revoking the final credential does not disable authentication. Neither activation nor recovery is exposed to clients yet; do not invoke the primitive from a production adapter until the transport boundary above is complete.
