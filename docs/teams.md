...
The Artifact handoff is not a security or context-isolation boundary. Provider-native launch controls may separately restrict each role; the [capability and evidence matrix](../public-docs/capabilities.md) owns the recorded provider/platform boundary proof and its limits. Run records keep step status, timestamps, agent IDs, frozen configuration, and bounded errors; agent timelines remain authoritative for output.
