...
For Codex-backed CC dogfood and real-provider QA, use `gpt-5.3-codex-spark` with low reasoning.
If Spark is unavailable or cannot exercise the required capability, stop and ask before using
another model; do not fall back to mini or a larger model. For other providers, select the cheapest
available model that supports the capability being tested. Keep prompts and turn counts small,
