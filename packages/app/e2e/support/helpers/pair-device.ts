...
export async function expectRelayConsent(page: Page): Promise<void> {
  ...
  await expect(
    modal.getByRole("link", { name: "Read how Consolidated Compute relay works" }),
  ).toBeVisible();
...
export async function expectRelayUpdateRequired(page: Page): Promise<void> {
  const modal = page.getByTestId("host-page-pair-device-card");
  await expect(
    modal.getByText("Update the host to enable relay from Consolidated Compute Desktop."),
  ).toBeVisible();
...
export async function openRelaySecurityDocs(page: Page): Promise<void> {
  await page.getByRole("link", { name: "Read how Consolidated Compute relay works" }).click();
  await expect
