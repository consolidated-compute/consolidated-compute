...
describe("getDaemonManagementErrorPresentation", () => {
  it("refreshes status when the daemon started but localhost registration failed", () => {
    ...
    expect(presentation).toEqual({
      message:
        "Built-in daemon started, but Consolidated Compute could not save the localhost connection. Toggle daemon management off and on again, or add localhost manually.",
      refreshStatus: true,
    });
  });
  ...
  it("does not refresh status for daemon stop failures", () => {
    ...
    expect(presentation).toEqual({
      message:
        "Built-in daemon management was paused, but Consolidated Compute could not stop the daemon.",
      refreshStatus: false,
    });
  });
