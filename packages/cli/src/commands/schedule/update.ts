...
export async function runUpdateCommand(
  ...
  try {
    const schedule = await requireStandaloneSchedule(client, id);
    if (input.newAgentConfig && schedule.target.type !== "new-agent") {
      throw new Error("Agent launch options can only update a new-agent schedule");
    }
